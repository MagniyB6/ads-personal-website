...
def handler(event: dict, context) -> dict:
    """CRUD для отчётов клиентам: создание, получение, обновление, удаление"""
    ...
    # Очищаем устаревшие отчёты
    cur.execute(f"DELETE FROM {SCHEMA}.report_blocks rb USING {SCHEMA}.reports r WHERE rb.report_id = r.id AND r.expires_at < NOW()")
    cur.execute(f"DELETE FROM {SCHEMA}.reports WHERE expires_at < NOW()")
    conn.commit()

    try:
        # GET /reports?id=... — получить отчёт
        if method == "GET" and report_id:
            ...
            # Скрываем edit_token для публичного просмотра
        ...
        # PUT /reports?id=... — обновить отчёт (требует edit_token)
